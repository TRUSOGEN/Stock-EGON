...
如果你想一次性填持仓、邮箱、LLM 投研助理和新闻源，可以打开 [docs/config-wizard.html](docs/config-wizard.html)。这个页面会在浏览器本地生成 `PORTFOLIO_JSON`、Secrets `.env` 内容和一键终端脚本。生成后的脚本可以直接复制到本机终端执行，macOS 默认 zsh 或 bash 都可以；如果终端提示没有登录 GitHub CLI，先运行 `gh auth login`。
...
| `ALPHA_VANTAGE_API_KEY` | Alpha Vantage News & Sentiment key | 新闻推荐 | Secrets |
