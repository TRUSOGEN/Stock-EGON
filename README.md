...
第一步，把仓库保留在 GitHub 私有仓库里，然后打开 `Actions` 页面启用 `US Stock Portfolio Report` workflow。这个 workflow 会在北京时间周二到周六 08:17、08:37、08:57 左右尝试生成日报，在北京时间周六 09:17、09:37、09:57 左右尝试生成周报，也可以手动运行。自动补跑会用当天 marker artifact 去重，已成功发送过的同类型报告不会重复发送。GitHub Actions 的定时任务仍可能延迟或偶发漏触发，若当天仍没有收到邮件，可以在 `Actions` 页面手动运行一次 `daily`。
