...
本工具用于把公开行情数据转成结构化研究结果，帮助快速完成市场概览、候选筛选、个股技术诊断、经典策略回测、美股持仓日报和每周复盘。工具输出是研究参考，不是投资建议、交易指令或合规结论。
...
真实持仓、新闻源 token、SMTP 密码或邮箱授权码和通知 webhook 应通过 GitHub Secrets、Codex automation prompt 或运行时环境变量注入，不应写入仓库。`config/portfolio.example.json` 和文档示例只能使用假持仓。
...
美股持仓日报关注前一交易日收盘后的组合状态，核心输出包括组合净值、现金权重、增持候选、减持候选、继续持有、重点观察、每只持仓的一句话解释和数据限制。每周复盘关注组合集中度、下周事件风险、动作复核和观察清单。评分由趋势、动量、组合集中度、新闻风险和持仓盈亏共同决定。
...
通知层只发送已经生成的报告，不参与评分、风控和数据抓取。默认 workflow 使用 SMTP 邮件推送；QQ 邮箱可通过 `EMAIL_ADDRESS` 和 `EMAIL_AUTH_CODE` 使用内置默认值，其他邮箱使用完整 SMTP 配置。企业微信群机器人 webhook 只作为可选保留通道。交互式微信问答需要独立的消息接收服务、鉴权、审计和 agent API 调用链路。
